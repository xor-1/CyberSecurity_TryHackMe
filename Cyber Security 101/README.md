...
- **Machines:** Machines are also objects in the AD, and also considered as **Security Principles** because they are also assigned an account and authenticated when connected to the network but they have very limited access. Machine accounts are not accessed by any other accounts (Local Administrators) whereas other accounts use can use them if you have password.
- 